...
printf "Für wen möchtest du das Board einrichten ?\n"
printf "(1) Hugo\n"
printf "(2) Philip\n"
...
if [[ "$x" != "1" && "$x" != "2" ]]; then
    printf "Ungültiger Eingabewert! Das Programm wird geschlossen.\n"
    exit 1
fi
...
if [[ "$x" == "1" ]]; then
    ...
    printf "Die Dartscheibe wurde erfolgreich für Hugo konfiguriert!\n"
fi
...
if [[ "$x" == "2" ]]; then
    ...
    printf "Die Dartscheibe wurde erfolgreich für Philip konfiguriert!\n"
fi
